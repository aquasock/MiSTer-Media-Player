...
function automatic signed [8:0] chroma_half_vector;
    input signed [8:0] v; reg signed [9:0] a;
...
(* ramstyle = "M10K" *) reg [37:0] motion_mem [0:MAX_MB-1];
reg [10:0] motion_count;
reg [37:0] motion_word;
reg motion_load;
reg motion_first_pending;
reg [1:0] pending_direction;
reg signed [8:0] pending_fmvx,pending_fmvy;
wire [1:0] mb_direction=motion_word[37:36];
wire signed [8:0] mb_fmvx=$signed(motion_word[35:27]);
wire signed [8:0] mb_fmvy=$signed(motion_word[26:18]);
wire signed [8:0] mb_bmvx=$signed(motion_word[17:9]);
wire signed [8:0] mb_bmvy=$signed(motion_word[8:0]);
...
(* preserve *) reg signed [8:0] exec_fmvx,exec_fmvy;
(* preserve *) reg signed [8:0] exec_bmvx,exec_bmvy;
(* preserve *) reg signed [8:0] phase_mvx,phase_mvy;
...
wire signed [8:0] exec_mvx=phase_mvx;
wire signed [8:0] exec_mvy=phase_mvy;
