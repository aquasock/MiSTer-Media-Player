...
localparam [1:0] Y=0,CB=1,CR=2;localparam [28:0] YB=29'h06000000,CBB=29'h0600A8C0,CRB=29'h0600D2F0,BANK=29'h00010000;
wire tcb=(pixel_component==Y)&&(pixel_x[11:10]==1),tcr=(pixel_component==Y)&&(pixel_x[11:10]==2),tag=tcb||tcr;wire [1:0] ec=tcb?CB:tcr?CR:pixel_component;wire [11:0] ex=tag?{2'b00,pixel_x[9:0]}:pixel_x;
...
always @(posedge clk)begin if(reset)begin cap<=0;flush<=0;writing<=0;ab<=0;ac<=0;ox<=0;oy<=0;wr<=0;wa<=0;sh<=0;block_stored<=0;write_seen<=0;store_error<=0;end else begin block_stored<=0;
if(block_start)begin if(cap||flush||writing)store_error<=1;cap<=1;ac<=ec;ab<=frame_bank;ox<={ex[11:3],3'b000};oy<={pixel_y[11:3],3'b000};end
