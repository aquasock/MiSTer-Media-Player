// Auto-detect a Program Stream pack prefix, preserving raw elementary input.
// All ports use ordinary valid/ready; the existing metadata extractor converts
// the output to the decoder's accepted-byte pulse contract. Audio is drained
// in this video-only milestone. PES timestamps are retained by the demux for
// subsequent A/V integration; presentation currently uses encoded cadence.
module mpeg2_program_stream_ingress (
...
mpeg2_h262_program_stream_demux demux (
    ...
    .video_pts(), .video_pts_valid(),
    .audio_data(), .audio_valid(), .audio_ready(1'b1),
    .audio_pts(), .audio_pts_valid(), .stream_end(), .demux_error(demux_error)
);
