...
wire mixed_detect_now=p_residual_sample_valid&&(p_residual_sample_index==6'h3e)&&
...
wire mixed_select=mixed_mode||mixed_detect_now||mixed_active;
always @(posedge clk)begin
 if(reset)mixed_mode<=0;
 else if(mixed_detect_now)mixed_mode<=1;
 else if(mixed_persisted_seen)mixed_mode<=0;
end
...
assign read_seen=mixed_select?mix_read:base_read;
assign sample_value=mixed_select?mix_sample:base_sample;
assign sample_nonzero=mixed_select?mix_nonzero:base_nonzero;
assign half_sample_seen=mixed_select?mixed_half:base_half;
assign reconstructed_seen=mixed_select?mix_recon:base_recon;
assign reconstructed_value=mixed_select?mix_recon_val:base_recon_val;
assign persisted_seen=mixed_select?mixed_persisted_seen:base_persisted_seen;
assign persisted_value=mixed_select?mix_persist_val:base_persist_val;
assign probe_error=mixed_error||(!mixed_select&&base_probe_error);
endmodule
