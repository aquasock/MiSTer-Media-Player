                ...
                if((motion_count==0)||motion_first_pending||metadata_done||!geometry_seen)begin error<=1;if(!error)error_source<=5'd6;end else metadata_done<=1;
                    ...
                    if(verify_row==7) begin
                        ...
                        if(blk==5) begin
                            if((col+1'b1>=mb_width)&&(mrow+1'b1>=mb_height)) begin
                                ...
                                if(mbi+1'b1!=motion_count)begin error<=1;if(!error)error_source<=5'd15;end
