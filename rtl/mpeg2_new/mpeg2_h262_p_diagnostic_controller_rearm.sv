...
//
// Phase 1U-n preserves the accepted zero/skipped and aligned raster paths and
// re-arms only the aligned per-picture plan/hold state when a consecutive P
// header arrives.  Long-lived sequence/residual diagnostics remain untouched.
//
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (!aligned_seen && aligned_candidate && aligned_plan_done) begin
            aligned_plan_sending <= 1'b0;
            aligned_plan_done    <= 1'b0;
            aligned_plan_index   <= 6'd0;
        end
        else if (aligned_seen && !aligned_plan_sending && !aligned_plan_done) begin
...
wire syntax_error =
    syntax_error_raw && !two_mb_seen && !four_mb_seen &&
    !aligned_candidate && !aligned_seen;
