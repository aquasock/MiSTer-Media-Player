...
module media_audio_rate_control #(
 ...
 always @(posedge clk)begin
  if(reset)begin state<=RUN;clock_cd<=0;config_mode<=0;error<=0;dirty<=0;quiet_count<=0;end
  else begin
   ...
   case(state)
    ...
    DRAIN:if(clients_idle)begin clock_cd<=want_cd;config_mode<=wanted_mode;state<=SELECT;end
