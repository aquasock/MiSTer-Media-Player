...
MiSTer-Phosphor is a simple, lightweight video and audio player, implemented entirely in FPGA logic. The primary goal of this project is to provide the user with an all-in-one solution to local video and audio playback on the MiSTer platform. Video playback is driven by a custom built, feature incomplete, MPG2 video and MP2 audio decoder. The main bulk of the FPGA logic lives here.  Audio playback is driven by an integrated FLAC decoder. Audio visualization are produced from RTL and driven by the MPG2 video decoder.
...
- Assume all user commands are run from the local project root.
...
7. I will inform you when my test results are ready in the local project folder.
