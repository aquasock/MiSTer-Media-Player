...
Reload the deployed core and run `test_compat_long_gop.m2v` once. Confirm whether visible presentation advances beyond frame 47 through the final source frame 71, USER and POWER remain solid, and the settled DISK report reaches stage eleven. If it stops early, report the last visible frame and the blink state of all three LEDs. Then reload `test_compat_mixed_macroblocks.m2v` and report whether its existing load-time jitter changes.
