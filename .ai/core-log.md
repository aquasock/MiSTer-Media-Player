...
The user rejects a diagnostic-only FPGA build because the accepted fit already consumes 532 of 553 M10Ks and timing is placement-sensitive, then approves a revised single-build cycle.  Do not add synthesized telemetry or produce a diagnostic RBF.  Recover the established live-raster simulation harness only into temporary build-PC storage and compile it against current functional source `0a9d48a`.  Exact three-second natural-content windows around the two hardware failures complete without decoder, reconstruction, writer or presentation errors but spend 101,646,454 Coming-to-America cycles and 122,096,396 Lebowski cycles in B raster occupancy; their registered retained-footprint lookup path alone occupies 94,990,954 and 115,440,896 cycles.  Exact complete-GOP reductions reproduce that split: Coming requires 23,329,997 cycles with 11,619,122 B lookup cycles, while Lebowski requires 24,739,997 with 15,991,054.  A temporary simulation-only removal of the lookup address register reduces those GOPs to 18,439,997 and 19,959,997 cycles respectively, with identical DDR-read counts and every functional error flag clear, proving that the two-cycle lookup acceptance bubble contains enough margin for both streams.  Historical timing evidence nevertheless rejects that temporary form: the pre-registered retained-word path missed decoder timing by more than 2 ns, whereas the current seed-19 build has only 0.462 ns decoder slack.  The authorized production boundary is therefore a streaming ordered lookup-issue cursor in the B raster engine plus a throughput-one registered mode in the existing footprint fetcher.  It must retain both address and data timing registers, begin streaming only after the selected retained footprint is complete so no speculative invalid response can duplicate a lookup, preserve ordered response-side reconstruction state, and add only flip-flop/ALM control with zero new M10Ks.  No synthesized counters, FIFO growth, reconstruction-arithmetic change, unregistered retained-word mux or automatic fitter reseed is authorized.

#### Next Steps:

Implement the bounded streaming cursor only in `rtl/mpeg2_new/mpeg2_h262_b_bidirectional_raster_engine_part1.svh`, `rtl/mpeg2_new/mpeg2_h262_b_bidirectional_raster_engine_part2.svh`, `rtl/mpeg2_new/mpeg2_h262_b_bidirectional_raster_engine_part3.svh` and `rtl/mpeg2_new/mpeg2_h262_prediction_block_fetcher.sv`.  First prove byte ordering, completion, exact reconstruction, frame/field motion, field-DCT, cadence, reorder, timestamp and overlap behavior entirely in simulation, including the two exact natural-content windows, and require both to clear the calculated 1.207 MB/s real-time input boundary.  Stop without compiling if the registered implementation does not meet that gate.  Otherwise commit only passing source, pull that exact commit onto build PC `10.10.0.42`, and perform exactly one clean Quartus 17.0.2 build at the pinned seed 19, requiring zero negative timing slack, zero new M10Ks and no total increase above 532 M10Ks.  If seed 19 fails timing, stop and report rather than reseeding automatically; do not alter Main, helper, media, the installed test-MiSTer artifact or the paused ISO deployment before a timing-clean result exists.
