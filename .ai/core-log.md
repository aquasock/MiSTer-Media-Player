...
Replace the permanent B-seen diagnostic gating with ownership-qualified error reporting now that the P controller exports only functional and unsupported-picture errors, and add a directed regression proving a deliberately injected I error after an accepted B picture reaches `probe_error`.  Require the corrected field-DCT fixture, scheduler suite and exact 6,751,008-byte replay to remain clean under the unmasked diagnostic, then run the established mixed, field-motion, field-DCT and original DVD regressions.  If those gates pass, perform one clean Quartus seed-20 build with focused decoder and video timing audit; if timing fails, stop without retrying or reseeding.  Install and hardware-test a resulting timing-clean RBF only after readback verification.
