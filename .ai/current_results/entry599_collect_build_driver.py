...
work=Path('/home/vash/mister-builds/entry599/FPGA')
subprocess.run(['git','diff','--exit-code','HEAD'],cwd=work,check=True,capture_output=True)
run=json.loads((work/'entry599-build-run.json').read_text())
assert run.get('exit_code')==0,run
log=(work/'entry599-quartus.log').read_text()
...
manifest={**run,'quartus_version':'17.0.2 Build 602 Lite','seed':int(seed[1]) if seed else None,'compile_success':True,'errors':int(counts[-1][0]),'warnings':int(counts[-1][1]),'timing_positive':positive,'worst_slack_ns':worst,'timing_rows':rows,'resources':resources,'rbf_bytes':rbf.stat().st_size,'rbf_sha256':hashlib.sha256(rbf.read_bytes()).hexdigest(),'report_sha256':{name:hashlib.sha256((work/name).read_bytes()).hexdigest() for name in ('entry599-quartus.log','output_files/MediaPlayer.sta.summary','output_files/MediaPlayer.fit.summary')}}
baseline_log=Path('/home/vash/mister-builds/entry588/FPGA/entry588-quartus.log').read_text(errors='replace')
...
(work/'entry599-build.json').write_text(json.dumps(manifest,indent=2)+'\n')
