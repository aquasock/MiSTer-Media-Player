...
root=Path('/run/media/vash/GIT/MiSTer-Media-Player');out=Path('/tmp/entry605-capture');out.mkdir(exist_ok=True)
m={'captured':datetime.now(ZoneInfo('America/Phoenix')).isoformat(timespec='seconds'),'host':'10.10.0.30','candidate':'d466bed','expected_rbf_sha256':'d46f80061a3270c1fed07a089517e70b413d3353858dc0d8937ac1bb0070aa6a','expected_media_sha256':'beb5c738910321fbbdf482220c19af36e7c2d2bb1913e8872f679eeb1f589642','expected_media_bytes':739065873,'user_report':None,'mode':None,'files':{},'operations':[]}
def connect():
...
f=connect()
helper=retrieve(f,'/tmp/MediaPlayer_ARM.log','entry605_arm_helper.log')
...
time.sleep(2)
retrieve(f,shot,'entry605_terminal.png');f.quit()
...
(out/'entry605_capture.json').write_text(json.dumps(m,indent=2)+'\n')
sys.path.insert(0,str(root/'tools/streams'))
from decode_hardware_cadence import decode
r=decode(out/'entry605_terminal.png');(out/'entry605_terminal.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
