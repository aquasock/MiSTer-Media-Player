...
root=Path('/run/media/vash/GIT/MiSTer-Media-Player');out=Path('/tmp/entry630-capture');out.mkdir(exist_ok=True)
m={'captured':datetime.now(ZoneInfo('America/Phoenix')).isoformat(timespec='seconds'),'host':'10.10.0.30','candidate':'d466bed','expected_rbf_sha256':'61a2fed28425a461c8b886bdf809e3ef76a320e5688bb22a816135c36ef981ce','expected_media_sha256':'unknown-other-agent-fixture','expected_media_bytes':5410833,'user_report':'test 6 DTS ran; menu laggy; no woofer on the AV amp unlike test 5 AC-3','mode':None,'files':{},'operations':[]}
def connect():
...
f=connect()
helper=retrieve(f,'/tmp/MediaPlayer_ARM.log','entry630_arm_helper.log')
...
time.sleep(2)
retrieve(f,shot,'entry630_terminal.png');f.quit()
# 3. installed-artifact readbacks
f=connect()
for remote,name in [('/media/fat/MiSTer','MiSTer'),('/media/fat/MediaPlayer.rbf','MediaPlayer.rbf'),('/media/fat/games/MediaPlayer/test_6_audio_dts_51.mpg','fixture.mpg')]:
 try:retrieve(f,remote,name)
 except Exception as e:m['files'][remote]={'error':repr(e)}
f.quit()
rbf=m['files'].get('/media/fat/MediaPlayer.rbf',{})
med=m['files'].get('/media/fat/games/MediaPlayer/test_6_audio_dts_51.mpg',{})
...
(out/'entry630_capture.json').write_text(json.dumps(m,indent=2)+'\n')
sys.path.insert(0,str(root/'tools/streams'))
from decode_hardware_cadence import decode
r=decode(out/'entry630_terminal.png');(out/'entry630_terminal.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
