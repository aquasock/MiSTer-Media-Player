...
module tb_entry603_i_pts;
    localparam integer MAX_STREAM_BYTES = 8388608;
    ...
    always @(posedge clk) begin
        if (!reset) begin
            ...
            if(scheduler.ordinary_secondary_valid && pts_active)
            ...
            if(presented==100 && stream_index==stream_len && stores==100*8100)begin
                if(completed!=100||words!=100*64800||headers!=100)$fatal(1,"incomplete stream");
            ...
            if(total_cycles>400000000)$fatal(1,"timeout pictures=%0d presented=%0d byte=%0d stores=%0d",completed,presented,stream_index,stores);
        end
    end
endmodule
