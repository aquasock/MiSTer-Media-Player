...
root=Path('/run/media/vash/GIT/MiSTer-Media-Player');out=Path('/tmp/entry613-capture');out.mkdir(exist_ok=True)
m={'captured':datetime.now(ZoneInfo('America/Phoenix')).isoformat(timespec='seconds'),'host':'10.10.0.30','candidate':'d466bed','expected_rbf_sha256':'d46f80061a3270c1fed07a089517e70b413d3353858dc0d8937ac1bb0070aa6a','expected_media_sha256':'6adb420921791b33ea16cf880df64fe9b5665bd7cc3ae857d65f5036de9902e2','expected_media_bytes':15333393,'user_report':'sweep pattern heard correctly on both HDMI and S/PDIF; earlier HDMI silence was monitor volume','mode':None,'files':{},'operations':[]}
def connect():
...
f=connect()
helper=retrieve(f,'/tmp/MediaPlayer_ARM.log','entry613_arm_helper.log')
...
time.sleep(2)
retrieve(f,shot,'entry613_terminal.png');f.quit()
...
(out/'entry613_capture.json').write_text(json.dumps(m,indent=2)+'\n')
sys.path.insert(0,str(root/'tools/streams'))
from decode_hardware_cadence import decode
r=decode(out/'entry613_terminal.png');(out/'entry613_terminal.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
