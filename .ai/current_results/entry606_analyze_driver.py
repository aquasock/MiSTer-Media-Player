...
out=Path('/tmp/entry606-capture')
t=json.load(open(out/'entry606_terminal.json'))
cap=json.load(open(out/'entry606_capture.json'))
log=(out/'entry606_arm_helper.log').read_text(errors='replace').splitlines()
a={}
...
(out/'entry606_analysis.json').write_text(json.dumps(a,indent=2,sort_keys=True)+'\n')
