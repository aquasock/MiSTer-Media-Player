...
module tb_entry599_writer_pixels;
    ...
    mpeg2_h262_picture_bookkeeper parser(
        ...
        .pipeline_block_done(writer_accepted),
    ...
    always @(posedge clk) begin
        ...
        if(syntax_error||probe_error||iq_error||unsupported_matrix||idct_error||recon_error||writer_error)
        ...
        if(expect_reject&&(stream_index==stream_len)&&quiet_cycles>1000)begin
            ...
            $display("PASS tb_entry599_writer_pixels reject");
            $finish;
        end
        if(!expect_reject&&(stream_index==stream_len)&&
           (picture_count==FRAME_COUNT)&&quiet_cycles>1000)begin
            ...
            if(!phase1_seen||
               ...
               pixel_mismatches!=0||max_pixel_delta>1||written_words!=FRAME_COUNT*64800||stored_blocks!=FRAME_COUNT*8100)
                $fatal(1,"I reconstruction regression failed");
            if(!expect_progressive&&writer_busy_length==0&&
               total_cycles>REALTIME_2997_FOUR_FRAME_CYCLES)
                $fatal(1,"interlaced all-I throughput missed 29.97 fps: %0d cycles",
                    total_cycles);
            $display("PASS tb_entry599_writer_pixels accept");
            $finish;
        end
    end
