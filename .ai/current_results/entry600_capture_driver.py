...
root=Path('/home/vash/MiSTer-Media-Player');out=Path('/tmp/entry600-capture');out.mkdir(exist_ok=True)
m={'captured':datetime.now(ZoneInfo('America/Phoenix')).isoformat(timespec='seconds'),'host':'10.10.0.30','user_report':'menu is ok. the screen is ready for you.','files':{},'operations':[]}
def connect():
...
f=connect()
helper=retrieve(f,'/tmp/MediaPlayer_ARM.log','entry600_ceiling_arm_helper.log')
...
time.sleep(2)
retrieve(f,shot,'entry600_ceiling_terminal.png');f.quit()
f=connect()
...
(out/'entry600_capture.json').write_text(json.dumps(m,indent=2)+'\n');print(json.dumps(m,indent=2))
sys.path.insert(0,str(root/'tools/streams'))
from decode_hardware_cadence import decode
r=decode(out/'entry600_ceiling_terminal.png');(out/'entry600_ceiling_terminal.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
