...
root=Path('/run/media/vash/GIT/MiSTer-Media-Player');out=Path('/tmp/entry612-capture');out.mkdir(exist_ok=True)
m={'captured':datetime.now(ZoneInfo('America/Phoenix')).isoformat(timespec='seconds'),'host':'10.10.0.30','candidate':'d466bed','expected_rbf_sha256':'d46f80061a3270c1fed07a089517e70b413d3353858dc0d8937ac1bb0070aa6a','expected_media_sha256':'fb71dee0a1af7746809505b73ccc432f5951fbf01579ca63bbcb90e6c7aefece','expected_media_bytes':12787729,'user_report':'AC-3 10s clip: nothing from HDMI, mixed tones from AV receiver over S/PDIF','mode':None,'files':{},'operations':[]}
def connect():
...
f=connect()
helper=retrieve(f,'/tmp/MediaPlayer_ARM.log','entry612_arm_helper.log')
...
time.sleep(2)
retrieve(f,shot,'entry612_terminal.png');f.quit()
# 3. installed-artifact readbacks
f=connect()
for remote,name in [('/media/fat/MiSTer','MiSTer'),('/media/fat/MediaPlayer.rbf','MediaPlayer.rbf'),('/media/fat/games/MediaPlayer/ac3_480i_tff_5p1_10s.mpg','fixture.mpg')]:
 try:retrieve(f,remote,name)
 except Exception as e:m['files'][remote]={'error':repr(e)}
f.quit()
rbf=m['files'].get('/media/fat/MediaPlayer.rbf',{})
med=m['files'].get('/media/fat/games/MediaPlayer/ac3_480i_tff_5p1_10s.mpg',{})
...
(out/'entry612_capture.json').write_text(json.dumps(m,indent=2)+'\n')
sys.path.insert(0,str(root/'tools/streams'))
from decode_hardware_cadence import decode
r=decode(out/'entry612_terminal.png');(out/'entry612_terminal.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
