...
module tb_entry604_i_pts;
    localparam integer MAX_STREAM_BYTES = 67108864;
    ...
    integer use_writer=1,phase=301711,busy_period=0,busy_length=0;
    ...
    reg [31:0] metadata_positions[0:2047];
    reg [32:0] metadata_values[0:2047];
    ...
    mpeg2_h262_b_presentation_scheduler scheduler(
        ...
        .frame_rate_code(frame_rate_code),.timestamp_candidate_active(pts_active),
        .timestamp_candidate_due(pts_due),.native_ordinary_overlap_enable(control!=2),
        ...
        .cadence_slot_debug(cadence_slot),.candidate_frame_valid(candidate_valid),
    ...
    initial begin
        ...
        if($value$plusargs("PICTURES=%d",expected_pictures))begin end
    ...
    always @(negedge clk) begin
        if (!reset) begin
            ...
            if(control!=1 && metadata_index<metadata_count && stream_index==metadata_positions[metadata_index])begin
    ...
    always @(posedge clk) begin
        if (!reset) begin
            ...
            if(scheduler.ordinary_secondary_valid && pts_active && presentation_error)
            ...
            if(presented==expected_pictures && stream_index==stream_len && stores==expected_pictures*8100)begin
                if(completed!=expected_pictures||words!=expected_pictures*64800||headers!=expected_pictures)$fatal(1,"incomplete stream");
            ...
            if(total_cycles>2140000000)$fatal(1,"timeout pictures=%0d presented=%0d byte=%0d stores=%0d",completed,presented,stream_index,stores);
        end
    end
endmodule
