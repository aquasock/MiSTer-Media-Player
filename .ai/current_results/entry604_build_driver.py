...
if kind=='prepare':
 ...
 path=work/'entry604-build-run.json';path.write_text(json.dumps(report,indent=2)+'\n');start=time.monotonic()
 with (work/'entry604-quartus.log').open('w') as log:report['exit_code']=subprocess.call(cmd,cwd=work,env=env,stdout=log,stderr=subprocess.STDOUT)
