...
root=Path('/run/media/vash/GIT/MiSTer-Media-Player');out=Path('/tmp/entry632-capture');out.mkdir(exist_ok=True)
m={'captured':datetime.now(ZoneInfo('America/Phoenix')).isoformat(timespec='seconds'),'host':'10.10.0.30','candidate':'d466bed','expected_rbf_sha256':'61a2fed28425a461c8b886bdf809e3ef76a320e5688bb22a816135c36ef981ce','expected_media_sha256':'unknown-other-agent-fixture','expected_media_bytes':3401745,'user_report':'test 1 after installing entry624 Main; menu perfect now','mode':None,'files':{},'operations':[]}
def connect():
...
f=connect()
helper=retrieve(f,'/tmp/MediaPlayer_ARM.log','entry632_arm_helper.log')
...
time.sleep(2)
retrieve(f,shot,'entry632_terminal.png');f.quit()
# 3. installed-artifact readbacks
f=connect()
for remote,name in [('/media/fat/MiSTer','MiSTer'),('/media/fat/MediaPlayer.rbf','MediaPlayer.rbf'),('/media/fat/games/MediaPlayer/test_1_interlace_tff.mpg','fixture.mpg')]:
 try:retrieve(f,remote,name)
 except Exception as e:m['files'][remote]={'error':repr(e)}
f.quit()
rbf=m['files'].get('/media/fat/MediaPlayer.rbf',{})
med=m['files'].get('/media/fat/games/MediaPlayer/test_1_interlace_tff.mpg',{})
...
(out/'entry632_capture.json').write_text(json.dumps(m,indent=2)+'\n')
sys.path.insert(0,str(root/'tools/streams'))
from decode_hardware_cadence import decode
r=decode(out/'entry632_terminal.png');(out/'entry632_terminal.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n')
