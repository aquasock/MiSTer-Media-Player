...
- an optional legal-H.262 audio visualizer pack: after ten seconds without playback input the interface clears to a seamless MPEG-2 loop whose four synchronized color grades follow decoded-PCM loudness; activity restores the interface and a missing pack retains the normal screen;
...
- The optional `/media/fat/linux/MediaPlayer_Visualizer.mmpvis` asset is generated with `python3 tools/generate-audio-visualizer.py host/build/MediaPlayer_Visualizer.mmpvis`. It contains four aligned three-picture closed-GOP MPEG-2 variants, not malformed decoder input. The helper sends one selected GOP at each source phase, limits each video admission to 4 KiB between PCM records, hides the two-bit player overlay after ten playback seconds without input, and restores it on pause/resume or seek activity. This prototype uses fixed RMS thresholds rather than per-track automatic gain.
