...
mp2_decoder dut(.clk(clk),.reset(reset),.input_data(data),.input_valid(iv),.input_ready(ir),.input_end(ie),
.input_pts(33'd90000),.input_pts_valid(ipt),.pcm_valid(pv),.pcm_ready(ready),.pcm_left(l),.pcm_right(r),
.pcm_pts(pts),.pcm_pts_valid(pts_valid),.error(error),.frames_decoded(frames),.idle(idle));
...
always @(posedge clk) begin
 ...
 if(pv&&ready) begin
  $fwrite(ofd,"%d %d\n",l,r);
  if(n%1152==0 && (!pts_valid||pts!=90000+(n/1152)*2160)) $fatal(1,"PTS mismatch");
  n<=n+1;
 end
end
initial begin
