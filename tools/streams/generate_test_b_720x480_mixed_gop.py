#!/usr/bin/env python3
"""Generate deterministic 720x480 mixed I/P/B hardware regression.

Coded order is I / P / B / P / B; display order is I / B / P / B / P.
The B pictures cover 45x30 macroblocks with forward, backward and bidirectional
prediction, safe signed half-sample vectors, internal skipped macroblocks, and
bounded Y0 residuals. Generated .m2v stays local-only.
"""
...
if __name__=='__main__':
 ...
 dst=Path(__file__).resolve().parent/'test_b_720x480_mixed_gop.m2v'
 with tempfile.TemporaryDirectory(prefix='mister_h262_b720_') as td:
 ...
 print(f'B0 skips={sorted(SKIPPED[0])}, residuals={sorted(B_RES[0])}')
 print(f'B1 skips={sorted(SKIPPED[1])}, residuals={sorted(B_RES[1])}')
