...
wire deadline_snapshot_ready;
integer deadline_fd,deadline_n;
reg [1023:0] deadline_path;
reg [31:0] deadline_saved_flags,deadline_saved_cycle,deadline_saved_bytes;
reg [31:0] deadline_expected_delay;
reg [31:0] deadline_retained_flags;
...
reg framebuffer_cache_write_valid=0;
reg framebuffer_cache_write_first_field=0;
reg [7:0] framebuffer_cache_write_addr=0;
...
mpeg2_h262_hardware_cadence_profiler #(
    ...
    .native_decode_active(native_decode_active),
    .decoder_input_pending(decoder_input_pending),
    .writer_capacity_blocked(writer_capacity_blocked),
    ...
    .framebuffer_cache_write_valid(framebuffer_cache_write_valid),
    .framebuffer_cache_write_first_field(framebuffer_cache_write_first_field),
    .framebuffer_cache_write_addr(framebuffer_cache_write_addr),
...
// Run the new payload alongside the complete legacy profiler regression.
mpeg2_h262_hardware_cadence_profiler #(
    .DEADLINE_DIAGNOSTICS(1'b1),
    .TERMINAL_SNAPSHOT_DELAY(32),
    .NO_PROGRESS_SNAPSHOT_DELAY(64),
    .OUTLIER_GAP_CYCLES(32'd10),
    .PROFILE_START_STC_SECONDS(14'd5)
    ...
    .snapshot_ready(deadline_snapshot_ready)
);
...
begin
    ...
    native_active=0;native_decode_active=0;decoder_input_pending=1;writer_capacity_blocked=0;
    ...
    framebuffer_cache_write_valid=0;
    framebuffer_cache_write_first_field=0;
    framebuffer_cache_write_addr=0;
    framebuffer_luma_return_valid=0;
...
begin
    ...
    if(checksum!==dut.snapshot_sync_2[2047:2016])
        $fatal(1,"checksum mismatch %h/%h",checksum,
               dut.snapshot_sync_2[2047:2016]);
end
endtask
...
begin
    ...
    y0=native_mode?12'd224:12'd344;
...
begin
    native_active=native_mode;
    v_pos=native_mode?12'd225:12'd345;
...
// Match scheduler NBA timing: its new bank becomes visible after the same
// rising edge which consumed the swap window, not before that edge.
task deadline_present;
    input [1:0] bank;
    begin
        @(negedge clk_mpeg2);swap_window_pulse=1;candidate_presentable=1;cadence_slot=1;
        @(posedge clk_mpeg2);#1;display_frame_bank=bank;
        @(negedge clk_mpeg2);swap_window_pulse=0;candidate_presentable=0;
        repeat(4)@(negedge clk_mpeg2);
    end
endtask
...
task deadline_miss;
    begin
        ...
        @(negedge clk_mpeg2);swap_window_pulse=0;
        @(posedge clk_mpeg2);#1;
        deadline_saved_cycle=deadlines.deadline_window_sample[1];
        deadline_saved_flags=deadlines.deadline_window_sample[2];
        deadline_saved_bytes=deadlines.deadline_window_sample[7];
        repeat(3)@(negedge clk_mpeg2);
    end
endtask
...
initial begin
    ...
    if(dut.snapshot_sync_2[63:32]!==32'h1240ea60)
    ...
    // Exact deadline evidence: a clean input queue can be empty while the
    // unrelated HPS FIFO is full. Capture the original boundary, not later
    // recovery state, and wait for presentation before counting the gap.
    reset_all();activate_session();native_decode_active=1;frame_rate_code=4;
    pulse_reference();pulse_reference();deadline_present(1);
    if(deadlines.deadline_gap_count!=0)$fatal(1,"on-time initial swap counted");
    @(negedge clk_mpeg2);decoder_input_pending=0;fifo_pending=1;
    repeat(9)@(negedge clk_mpeg2);
    candidate_presentable=0;cadence_slot=1;completed_frame_bank=1;
    deadline_miss();
    if(!deadlines.deadline_pending||deadlines.deadline_gap_count!=0)
        $fatal(1,"miss must remain unconfirmed until swap");
    if(deadline_saved_flags[23:20]!==4'b1011 ||
       deadline_saved_flags[14:13]!==2'b01 ||
       deadline_saved_flags[5:2]!==4'b0101 ||
       deadline_saved_flags[31:24]!==8'd0)
        $fatal(1,"deadline state packing wrong %h",deadline_saved_flags);
    if(deadlines.deadline_pending_sample[0]!=={16'd3,16'd2})
        $fatal(1,"deadline ordinal/reference count wrong");
    if(deadlines.deadline_pending_sample[4]!=11||deadlines.deadline_pending_sample[5]!=0)
        $fatal(1,"input starvation attribution wrong count=%0d",deadlines.deadline_pending_sample[4]);
    // Change every relevant flag after the deadline and reveal the candidate.
    @(negedge clk_mpeg2);decoder_input_pending=1;fifo_pending=0;
    decoder_ready=0;writer_capacity_blocked=1;writer_busy=1;
    candidate_presentable=1;completed_frame_bank=2;
    @(posedge clk_mpeg2);#1;
    deadline_expected_delay=deadlines.session_cycles-deadline_saved_cycle;
    repeat(4)@(negedge clk_mpeg2);
    pulse_reference();deadline_present(2);
    if(deadlines.deadline_gap_count!=1||
       deadlines.deadline_records[1]!=deadline_saved_cycle||
       deadlines.deadline_records[2]!=deadline_saved_flags||
       deadlines.deadline_records[6]!=deadline_expected_delay||
       deadlines.deadline_records[7]!=deadline_saved_bytes)
        $fatal(1,"deadline changed after recovery delay=%0d expected=%0d",deadlines.deadline_records[6],deadline_expected_delay);

    // Writer capacity attribution with a nonempty decoder queue; an already
    // ready picture denied by cadence records a zero candidate-ready delay.
    repeat(6)@(negedge clk_mpeg2);
    candidate_presentable=1;cadence_slot=0;
    deadline_miss();
    if(deadlines.deadline_pending_sample[4]!=0||deadlines.deadline_pending_sample[5]==0)
        $fatal(1,"writer capacity confused with input starvation");
    if(deadlines.deadline_pending_sample[6]!=0)$fatal(1,"ready-at-window delay not zero");
    writer_capacity_blocked=0;writer_busy=0;decoder_ready=1;
    pulse_reference();deadline_present(0);
    if(deadlines.deadline_gap_count!=2)$fatal(1,"second deadline record missing");

    // Preserve full ordinals through eight-bit wrap. On-time windows must not
    // allocate records, even though the old bank is sampled at the window.
    for(deadline_n=5;deadline_n<=347;deadline_n=deadline_n+1)begin
        pulse_reference();deadline_present((deadline_n-1)%3);
    end
    if(deadlines.display_picture_count_full!=347||deadlines.deadline_gap_count!=2)
        $fatal(1,"wide count or on-time window suppression failed");
    candidate_presentable=0;cadence_slot=1;
    deadline_miss();
    // Another missed window in the same gap must retain the first timestamp.
    deadline_saved_cycle=deadlines.deadline_pending_sample[1];
    pulse_admission_window();
    if(deadlines.deadline_pending_sample[1]!=deadline_saved_cycle)
        $fatal(1,"second missed window overwrote first boundary");
    pulse_reference();deadline_present(2);
    if(deadlines.deadline_gap_count!=3||
       deadlines.deadline_records[16]!=={16'd348,16'd347})
        $fatal(1,"full ordinal 348 was not retained");
    // Confirmed fourth gaps count, but cannot overwrite the first three.
    deadline_retained_flags=deadlines.deadline_records[18];
    candidate_presentable=0;deadline_miss();pulse_reference();deadline_present(0);
    if(deadlines.deadline_gap_count!=4||deadlines.deadline_records[18]!=deadline_retained_flags)
        $fatal(1,"retention overflow altered first records");
    // No further presentation: the unconfirmed terminal idle never counts.
    sequence_end_seen=1;candidate_presentable=0;
    deadline_miss();
    ...
    if($value$plusargs("DEADLINE_SNAPSHOT=%s",deadline_path))begin
        deadline_fd=$fopen(deadline_path,"w");
        for(i=0;i<64;i=i+1)$fdisplay(deadline_fd,"%08x",deadlines.snapshot_sync_2[i*32+:32]);
        $fclose(deadline_fd);
    end
    // A repeated load resets counters, records, and deferred evidence. Native
    // mode changes clear a pending interval without changing playback state.
    reset_all();activate_session();native_decode_active=1;frame_rate_code=4;
    pulse_reference();pulse_reference();deadline_present(1);
    candidate_presentable=0;deadline_miss();
    native_decode_active=0;repeat(4)@(negedge clk_mpeg2);
    if(deadlines.deadline_pending)$fatal(1,"mode exit retained a pending gap");
    native_decode_active=1;deadline_present(2);
    if(deadlines.deadline_gap_count!=0||deadlines.deadline_records[0]!=0)
        $fatal(1,"new session inherited deadline evidence");
    $display("DEADLINE_PROFILER_PASS exact-window input-vs-writer late-ready on-time ordinal348 terminal-idle reset");
...
initial begin
    repeat(20000)@(posedge clk_mpeg2);
