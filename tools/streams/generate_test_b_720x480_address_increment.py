#!/usr/bin/env python3
"""Generate deterministic 720x480 B macroblock-address regression.

Coded order is I / P / B / P / B; display order is I / B / P / B / P.
Each B slice starts with a coded macroblock, then uses an escape-accumulated
MBA increment of 34 and a normal Table-B.1 increment of 10 before ending on
macroblock 44. Selected coded B macroblocks retain multi-plane residuals so
Commit-170 residual coverage remains exercised. Generated .m2v stays local-only.
"""
...
MBA={
 ...
 9:'0000110',10:'00001011',11:'00001010',12:'00001001',13:'00001000',14:'00000111',15:'00000110',
 16:'0000010111',17:'0000010110',18:'0000010101',19:'0000010100',20:'0000010011',21:'0000010010',
 22:'00000100011',23:'00000100010',24:'00000100001',25:'00000100000',26:'00000011111',27:'00000011110',
 28:'00000011101',29:'00000011100',30:'00000011011',31:'00000011010',32:'00000011001',33:'00000011000'}
...
def p_row(r:int)->bytes:
 ...
 for _c in range(MBW):bits+='1'+'001'+'1'+'1'
...
def b_row(v:int,r:int)->bytes:
 ...
 for c in CODED_COLS:
  ...
  prev_c=c
 if prev_c!=MBW-1:raise AssertionError(prev_c)
 return bits_to_bytes(bits)
...
def patch_picture(data:bytes,pic_index:int,payloads:tuple[bytes,...])->bytes:
 ...
 if ptype==2:b[pce+5]=0x30|(b[pce+5]&0x0f)
  ...
  b[pce+5]=0x33;b[pce+6]=(b[pce+6]&0x0f)|0x30
 ...
 if tuple(c for _,_,c in rows)!=tuple(range(1,MBH+1)):raise SystemExit(f'picture {pic_index} type {ptype}: unexpected slice layout')
...
def patch(data:bytes)->bytes:
 ...
 b=data;b=patch_picture(b,4,B_ROWS[1]);b=patch_picture(b,3,P_ROWS);b=patch_picture(b,2,B_ROWS[0]);b=patch_picture(b,1,P_ROWS)
 return b
...
if __name__=='__main__':
 ...
 dst=Path(__file__).resolve().parent/'test_b_720x480_address_increment.m2v'
 with tempfile.TemporaryDirectory(prefix='mister_h262_bmba_') as td:
 ...
 print('B coded columns per row: 0, 34, 44')
 print('B MBA increments per row: 1, 34 (macroblock_escape + 1), 10')
