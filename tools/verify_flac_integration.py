#!/usr/bin/env python3
"""Production PCM CDC/HDMI integration with vendor FIFO and functional PLL/hard-gate model."""
...
platform=[str(s.relative_to(root)) for s in (root/'rtl/platform').glob('*.sv')]
...
(out/'summary.json').write_text(json.dumps({'scope':'production native wrapper, vendor FIFO, functional PLL/hard-gate model; physical HDMI/clock/hardware qualification pending','results':results},indent=2)+'\n')
