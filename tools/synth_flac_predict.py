...
p.add_argument('--top',choices=['flac_predict_mac','flac_subframe'],default='flac_predict_mac')
a=p.parse_args();out=a.output.resolve();out.mkdir(parents=True,exist_ok=True)
root=Path(__file__).resolve().parents[1];source=root/f'rtl/audio/flac/{a.top}.sv'
source_assignments='\n'.join(f'set_global_assignment -name SYSTEMVERILOG_FILE \"{path}\"' for path in sorted({source,root/'rtl/audio/flac/flac_predict_mac.sv'}))
