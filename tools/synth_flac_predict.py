#!/usr/bin/env python3
"""Isolated Cyclone V FLAC block fit; does not build/change the core."""
...
p.add_argument('--top',choices=['flac_predict_mac','flac_subframe','flac_stream_decoder','flac_ddr_decoder','media_hdmi_audio_control','media_pcm_i2s'],default='flac_predict_mac')
a=p.parse_args();out=a.output.resolve();out.mkdir(parents=True,exist_ok=True)
root=Path(__file__).resolve().parents[1];source=root/f'rtl/audio/flac/{a.top}.sv'
dependencies={source,root/'rtl/audio/flac/flac_predict_mac.sv'}
if a.top=='flac_stream_decoder':dependencies.add(root/'rtl/audio/flac/flac_subframe.sv')
if a.top=='flac_ddr_decoder':dependencies=set((root/'rtl/audio/flac').glob('*.sv'))
if a.top=='media_hdmi_audio_control':
 source=root/'rtl/platform/media_hdmi_audio_control.sv';dependencies=set((root/'rtl/platform').glob('*.sv'))
if a.top=='media_pcm_i2s':
 source=root/'rtl/audio/media_pcm_i2s.sv';dependencies={source,root/'rtl/audio/media_pcm_sink.sv'}
source_assignments='\n'.join(f'set_global_assignment -name SYSTEMVERILOG_FILE \"{path}\"' for path in sorted(dependencies))
...
set_global_assignment -name TOP_LEVEL_ENTITY {a.top}
{source_assignments}
...
result={'scope':'isolated primitive, virtual I/O; not a whole-core resource/timing result','top':a.top,'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest()}
result['sources_sha256']={str(path.relative_to(root)):hashlib.sha256(path.read_bytes()).hexdigest() for path in sorted(dependencies)}
